...
public class ChatService {
    ...
    private static String buildSystemPrompt() {
        ...
        return """
            ...
            [비용 항목 표시 원칙]
            수수료(중개이용료, 결제수수료)와 배달비는 플랫폼 정책상 조정 불가한 비용입니다.
            단, 수치는 반드시 표시하고 "(조정 불가)" 태그를 붙이세요. 인사이트는 생략해도 됩니다.
            아래 3가지는 사장님이 직접 조절 가능한 항목으로, 수치와 인사이트를 모두 제공하세요.

            [핵심 분석 항목 - 반드시 수치와 인사이트 포함]
            1. 광고비:
               - 매출 대비 비율 명시. 3% 이하 효율적 / 5% 초과 과다 지출로 조정 권장.
               - [미집행] 상태이면 "현재 광고를 집행하지 않고 있습니다. 우리가게클릭 광고 집행 시 노출 확대 효과를 기대할 수 있습니다."라고 반드시 언급.
            2. 재료비:
               - 원가율(%) 명시. 30% 이하 양호 / 35% 초과 메뉴 가격 조정 또는 원가 절감 검토 권장.
               - 3개월 트렌드 분석 시 월별 원가율 변화(%p)를 반드시 수치로 언급.
            3. 기타비용:
               - 비율이 3% 초과 시 어떤 지출인지 점검 권장.
